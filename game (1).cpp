        ...
        for (int j = 0; j < 8; j++) {
            ...
            if (fullRedraw || RectInRect(&cellRect, dirtyRect)) {
                ...
                if (board->board[i][j].type != EMPTY) {
                    ...
                    case KING: _tcscpy(pieceText, _T("K")); break;
                    case QUEEN: _tcscpy(pieceText, _T("Q")); break;
                    case ROOK: _tcscpy(pieceText, _T("R")); break;
                    case BISHOP: _tcscpy(pieceText, _T("B")); break;
                    case KNIGHT: _tcscpy(pieceText, _T("N")); break;
                    case PAWN: _tcscpy(pieceText, _T("P")); break;
                    }
...
void showNotation(ChessBoard* board) {
    ...
    while (!exitFlag) {
        ...
        while (PeekMessage(&winMsg, NULL, 0, 0, PM_REMOVE)) {
            if (winMsg.message == WM_QUIT) exitFlag = true;
...
const TCHAR* getPieceNameCN(PieceType type) {
    switch (type) {
    case PAWN:   return _T("兵");
    case KNIGHT: return _T("马");
    case BISHOP: return _T("象");
    case ROOK:   return _T("车");
    case QUEEN:  return _T("后");
    case KING:   return _T("王");
...
void showGameOver(GameState state) {
    ...
    switch (state) {
    case CHECKMATE:
        outtextxy(280, 280, currentPlayer == white ? _T("黑方胜利!") : _T("白方胜利!"));
        break;
    case STALEMATE:
        outtextxy(300, 280, _T("和棋!"));
        break;
    }

    // 添加关闭按钮
    setfillcolor(RGB(200, 200, 200));
    fillrectangle(350, 380, 450, 420);
    outtextxy(380, 390, _T("关闭"));
...
void getPossibleMoves(ChessBoard* board, int fromX, int fromY, PossibleMoves* result) {
    ...
    case KNIGHT: {
        const int knightMoves[8][2] = { {2,1},{2,-1},{-2,1},{-2,-1},
                                      {1,2},{1,-2},{-1,2},{-1,-2} };
        for (int i = 0; i < 8; i++) {
        ...
        for (int dx = -1; dx <= 1; dx += 2) {
            for (int dy = -1; dy <= 1; dy += 2) {
                for (int step = 1; step < 8; step++) {
                    ...
                    if (toX < 0 || toX >= 8 || toY < 0 || toY >= 8) break;

                    (*result).moves[(*result).count][0] = toX;
                    (*result).moves[(*result).count][1] = toY;
                    (*result).count++;

                    if (board->board[toX][toY].type != EMPTY) break;
                }
            }
        }
        break;
