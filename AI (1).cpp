...
float minimax(ChessBoard* board, int depth, bool isMaximizing, float alpha, float beta) {
    ...
    const auto& pieces = isMaximizing ?
        (board->ai.color == white ? board->whitePieces : board->blackPieces) :
        (board->ai.color == white ? board->blackPieces : board->whitePieces);
...
void showHint(ChessBoard* board) {
    if (board->hintCount >= MAX_HINTS) return;
...
void showEvaluation(ChessBoard* board) {
    ...
    fillrectangle(250, 250, 550, 350);  // 窗口高度保持350

    // 显示得分和优势方
    TCHAR text[100];
    settextcolor(BLACK);
    _stprintf(text, _T("白方得分: %.1f"), whiteScore);
    outtextxy(280, 260, text);
    _stprintf(text, _T("黑方得分: %.1f"), blackScore);
    outtextxy(280, 290, text);

    if (whiteScore > blackScore) {
        outtextxy(280, 320, _T("白方占优"));
    } else if (blackScore > whiteScore) {
        outtextxy(280, 320, _T("黑方占优"));
    } else {
        outtextxy(280, 320, _T("局势均衡"));
    }
    ...
    while (true) {
        if (MouseHit()) {
            ...
            if (msg.uMsg == WM_LBUTTONDOWN &&
                ...
                msg.y >= 380 && msg.y <= 420) 
            {
                board->needRedraw = true;  // 关键！标记需要重绘
                return;  // 直接退出函数
            }
        }
